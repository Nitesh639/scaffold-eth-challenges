...
contract Streamer is Ownable {
    ...
    function withdrawEarnings(Voucher calldata voucher) public onlyOwner {
        ...
        bytes memory _prefixed = abi.encodePacked("\x19Ethereum Signed Message:\n32",_hashed);
        bytes32 _prefixedHashed = keccak256(_prefixed);

        address _msgSigner = ecrecover(_prefixedHashed,voucher.sig.v,voucher.sig.r,voucher.sig.s);
        require(balances[_msgSigner] > voucher.updatedBalance,"Insufficient balance!");
